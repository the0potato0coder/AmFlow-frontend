...
          throw new Error(`Failed to fetch user data: ${response.status} ${response.statusText}. ${errorData}`);
        }
...
      setUpdateError(err.message || 'An unexpected error occurred during update.');
    }
  };
...
                  {renderProfileImage()}
                  {/* Only show Edit Profile button if not in edit mode */}
                  {!editMode && (
